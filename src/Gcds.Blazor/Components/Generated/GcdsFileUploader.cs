...
public sealed class GcdsFileUploader : Gcds.Blazor.Forms.GcdsBindableStringListComponent
{
    ...
    [Parameter] public object? Files { get; set; }
    ...
    [Parameter] public string? UploaderId { get; set; }
    ...
    protected override IEnumerable<KeyValuePair<string, object?>> GetComponentParameters()
    {
        ...
        if (Files is not null) yield return new("files", Files);
        ...
        if (Multiple is not null) yield return new("multiple", Multiple);
        if (Name is not null) yield return new("name", Name);
        if (Required is not null) yield return new("required", Required);
        if (UploaderId is not null) yield return new("uploader-id", UploaderId);
